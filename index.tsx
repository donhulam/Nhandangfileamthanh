...
class VoiceNotesApp {
  ...
  constructor() {
    ...
    this.recordingStatus.textContent = 'Sẵn sàng tải file lên';
  }
  ...
  private bindEventListeners(): void {
    ...
    this.uploadAudioButton.addEventListener('click', () => this.audioFileInput.click());
  ...
  private async processAudio(audioBlob: Blob, explicitMimeType?: string): Promise<void> {
    if (audioBlob.size === 0) {
      this.recordingStatus.textContent =
        'File âm thanh trống. Vui lòng thử lại.';
  ...
  private async processTranscriptionAndPolish(transcription: string): Promise<void> {
    try {
        ...
        if (polishedText && polishedText.trim() !== '') {
            ...
            if (detectedLanguage.includes('english')) {
                this.recordingStatus.textContent = 'Ghi chép đã được trau chuốt và dịch. Sẵn sàng cho file tiếp theo.';
            } else {
                this.recordingStatus.textContent = 'Ghi chép đã được trau chuốt. Sẵn sàng cho file tiếp theo.';
            }
  ...
  private markdownToPlainText(markdown: string): string {
    ...
    // Block Elements
    text = text.replace(/^#{1,6}\s+(.*)/gm, '$1'); // Headings
    text = text.replace(/^>\s?/gm, ''); // Blockquotes
    text = text.replace(/^\s*([-*]|\d+\.)\s+/gm, ''); // List items
  ...
  private async downloadAsDocx(): Promise<void> {
    ...
    if (notesToDownload.length === 0) {
        alert('Không có nội dung để tải xuống. Vui lòng tạo một ghi chép trước.');
        return;
    }
    ...
    notesToDownload.forEach((note, index) => {
        ...
        if (polishedNoteContent) {
            ...
            const plainText = this.markdownToPlainText(polishedNoteContent);
  ...
  private async handleAudioFileSelect(event: Event): Promise<void> {
    ...
    try {
      this.recordingStatus.textContent = `Đang xử lý file: ${file.name}`;
      const audioBlob = new Blob([file], { type: file.type });
      await this.processAudio(audioBlob, file.type);
    } catch (error) {
      console.error('Lỗi xử lý file âm thanh:', error);
      this.recordingStatus.textContent = 'Lỗi xử lý file âm thanh.';
      this.enableUploadButton();
    } finally {
      input.value = '';
    }
  ...
  private initializeAiChat(): void {
    ...
    this.chatSuggestionContainer = document.createElement('div');
    this.chatSuggestionContainer.className = 'chat-suggestion-container';

    const suggestions = [
      'Tóm tắt nội dung ghi chép này.',
      'Tạo thành Báo cáo cho ghi chép này.',
      'Liệt kê các công việc cần làm',
      'Tạo thành Kết luận cho ghi chép này.',
      'Tạo thành biên bản cho ghi chép này.',
      'Tạo thành Tin tức cho ghi chép này.'
    ];
    ...
    suggestions.forEach(text => {
        ...
        this.chatSuggestionContainer.appendChild(button);
    });
    this.chatMessages.appendChild(this.chatSuggestionContainer);
...
--- Ghi chép kết thúc ---
        ...
        this.addMessageToChatUI('Xin chào! Hiện tại bạn chưa có ghi chép nào để trò chuyện. Hãy tải lên một file âm thanh hoặc hỏi tôi một câu hỏi chung!', 'ai');
    }
